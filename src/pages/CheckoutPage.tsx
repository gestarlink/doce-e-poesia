...
import { ArrowLeft, MessageCircle, MapPin, CreditCard, QrCode, Banknote, Copy, CheckCircle2, Loader2 } from "lucide-react";
  ...
  const processPixPayment = async (pedidoId: string) => {
    ...
    try {
      ...
      if (data?.qr_code) {
        ...
        // Fallback if MP not configured - still show order success
        toast.info("Pix será confirmado manualmente pelo estabelecimento");
      }
    } catch (err: any) {
      console.error("Pix error:", err);
      toast.info("Pagamento Pix será confirmado manualmente");
    } finally {
  ...
  if (orderPlaced) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="w-full max-w-sm text-center animate-fade-up space-y-5">
          <div className="text-6xl mb-2">✅</div>
          <h1 className="text-2xl font-display font-bold">Pedido enviado!</h1>
          <p className="text-muted-foreground text-sm">Seu pedido foi recebido e está sendo processado.</p>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-lg py-6">
        ...
        {/* Payment method selection - iFood style vertical */}
        <div className="mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full h-12 text-base font-display" disabled={loading || !paymentMethod}>
